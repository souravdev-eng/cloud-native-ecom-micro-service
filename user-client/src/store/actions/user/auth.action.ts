...
export const loggedOut = createAsyncThunk('user/loggedout', async (_, { rejectWithValue }) => {
  try {
    await axios.post(
...
export const currentUser = createAsyncThunk('user/currentuser', async (_, { rejectWithValue }) => {
  try {
    const user: CurrentUserResponse = await axios.get(`${BASE_URL}/users/currentuser`, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
