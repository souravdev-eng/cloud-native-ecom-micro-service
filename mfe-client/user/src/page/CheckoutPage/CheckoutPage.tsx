...
const CheckoutForm = () => {
	...
	/* An order exists but isn't paid — the cart is already cleared by cart's
	 * OrderCreatedListener, so an empty cart here is expected, not an error.
	 * Send the user to the order instead of the "cart is empty" dead end. */
	if (items.length === 0 && !paymentSuccess && hasUnpaidOrder && order) {
		return (
			<Styled.PageContainer>
				<Styled.SuccessContainer>
					...
					<Styled.OrderId>
						Total: <span>{formatMoney(order.totalAmount, order.currency)}</span>
					</Styled.OrderId>
		...
		return (
			<Styled.PageContainer>
				<Styled.SuccessContainer>
					...
					<Styled.SuccessText>
						Payment received and the order is marked paid.
					</Styled.SuccessText>
					<Styled.OrderId>
		...
		<Styled.PageContainer>
				...
				{steps.map((label, index) => (
					<Styled.StepItem key={label}>
						...
						{index < steps.length - 1 && (
							<Styled.StepConnector completed={activeStep > index} />
						)}
					</Styled.StepItem>
				))}
			</Styled.StepperContainer>
			...
			<Styled.ContentWrapper>
				...
				<Styled.FormSection>
					...
					{activeStep === 1 && (
						<>
							...
							{/* Once the order exists, retrying pays it rather than
							    creating a second one. */}
							{hasUnpaidOrder && order && (
								<Styled.Note>
									<InfoOutlinedIcon sx={{ fontSize: 16 }} />
									<span>
				...
				<Styled.SummarySection>
					...
					{/* Shipping/tax are front-end estimates and the order service prices
					    in its own currency, so the charged amount can differ. */}
					<Styled.Note>
						<InfoOutlinedIcon sx={{ fontSize: 16 }} />
						<span>
							{order ? (
								...
								<>
									Shipping and tax are front-end estimates. The charged total is
									computed by the order service from its own product prices.
								</>
							)}
						</span>
					</Styled.Note>
