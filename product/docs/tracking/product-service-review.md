...
### 1. Individual product cache TTL is 5 SECONDS — effectively useless ✅ FIXED
...
- **Fix:** Now uses the new `cache.set()` abstraction which has a default TTL of 15 minutes.

### 2. No cache invalidation on writes ✅ FIXED
...
- **Fix:** Added `await cache.del()` in all three locations using the new `RedisCache` abstraction.
...
- **Fix:** Changed to `await mongoose.connect(...)` to properly wait for connection before proceeding.
...
**Update (implemented):** for this specific case we went with the simpler variant — the precondition is encoded as a query filter rather than a version field:
...
MongoDB guarantees atomicity for a single-document `findOneAndUpdate`, so the filter match + decrement happen as one operation with no TOCTOU gap. A null result means either the product was deleted or stock is insufficient; we ack and drop (logging both cases distinctly). A version field becomes worthwhile when the event payload carries an expected version ("apply only if still v7") — our payload does not, so adding one would be pure overhead.

One orthogonal concern remains open: **idempotency on RabbitMQ redelivery** (item G below). The filter-guard fix does not protect against double-processing the same message after a crash between `save` and `ack`. That is a separate fix (processed-message log with unique index).
...
You're already doing read-heavy work (`ProductAPIFeature`) separate from writes. The next step: the _read model_ and _write model_ don't need to be the same MongoDB collection. The ETL service already syncs Mongo → Postgres. You could:
...
| --- | --- | --- | --- | --- |
| P0 | Cache invalidation on write (update/delete/quantity) | `updateProduct.ts`, `deleteProduct.ts`, `productQuantityUpdate.ts` | Correctness — wrong data served | ✅ Done |
